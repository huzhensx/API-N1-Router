...
app = FastAPI(title="API-N1-Router", lifespan=lifespan)
...
if __name__ == "__main__":
    ...
    def port_in_use(port: int) -> bool:
        # 用 bind 测试端口占用，避免 connect_ex 的探测连接（连上即断）触发
        # Windows Proactor 的 WinError 64，导致 asyncio 关闭监听 socket、服务失效。
    ...
    try:
        ...
        ctypes.windll.user32.MessageBoxW(
            0, "API-N1-Router 已在运行中，请勿重复启动。", "提示", 0x30
        )
        sys.exit(0)
    ...
    def create_tray_icon():
        # 优先使用打包进去的 icon.ico 作为托盘图标（与 exe 图标一致）
        icon_path = APP_DIR / "icon.ico"
        if icon_path.exists():
            try:
                return Image.open(str(icon_path)).convert("RGBA")
            except Exception:
                logger.warning("加载托盘图标失败，使用默认图标")
